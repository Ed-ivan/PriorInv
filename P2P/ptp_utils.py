...
def diffusion_step(model, controller, latents, context, t, guidance_scale, low_resource=False,
                   ...
                   inversion_guidance=True, x_stars=None, i=0, noise_loss=None, is_dynamic_scale=False,dynamic_threshold=0.7,capture_noise=None,**kwargs):
    ...
    if is_dynamic_scale:
        ...
        score_ori = score
        score_mask = torch.where(score.abs()<threshold,0.0, 1.0)
        #TODO: thresold 需要换一下
...
def text2image_ldm(
    ...
    for i,t in enumerate(model.scheduler.timesteps):
        if noise_loss_list is None:
            latents = diffusion_step(model, controller, latents, context, t, guidance_scale)
        else:
            latents = diffusion_step(model, controller, latents, context, t, guidance_scale,noise_loss=noise_loss_list[i])
...
def get_time_words_attention_alpha(prompts, num_steps,
    ...
    # 对于 这个p2p 来说 就是 ['a cat standing on the ground', 'a silver cat sculpture standing on the ground']
