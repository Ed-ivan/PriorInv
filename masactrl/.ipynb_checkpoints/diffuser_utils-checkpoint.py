...
#TODO： 这里添加了一点的内容啊  , 是不是没有调好啊,
def linear_schedule_old(t, guidance_scale, tau1=0.05, tau2=0.4):
    t = t / 1000
    assert t<1.0,"here in linear_schedule , some thing is wrong!"
    if t <= tau1:
        return guidance_scale
    if t >= tau2:
        if not isinstance(guidance_scale, torch.Tensor):
            return 1.0
        else:
            return torch.ones_like(guidance_scale)
    gamma = (tau2 - t) / (tau2 - tau1) * (guidance_scale - 1.0) + 1.0

    return gamma
#update function for tensor
            ...
            if do_separate_cfg:
                ...
                if (i >= prox_step) and (prox == 'l0' or prox == 'l1'):
                    ...
                    if prox == 'l1':
                        ...
                        score_delta = torch.where(score_delta < 0, score_delta+threshold, score_delta)
                #TODO: 这里面还是得改一下啊 不能简单使用 threshold 
                if is_dynamic_scale:
                    dynamic_scale_mask = torch.where(score_delta.abs() > threshold, torch.tensor(1.0), torch.tensor(0.0))
                    guidance_scale_1 = (guidance_scale_1-1.0) * dynamic_scale_mask + 1.0
                guidance_scale_1=linear_schedule(t,guidance_scale_1)
